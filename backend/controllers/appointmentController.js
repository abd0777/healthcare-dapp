...
export const checkAppointmentSlots = async (req, res) => {
  try {
    ...
    if (!licenseNumber || !date) {
      return res
        .status(400)
        .json({ message: "licenseNumber and date are required" });
    }
    ...
    res
      .status(500)
      .json({ message: "Error checking appointment slots", error });
  }
};
...
export const appointmentHistory = async (req, res) => {
  try {
    ...
    const appointments = await Appointment.find({
      patientGovtId: req.user.patientGovtId,
    });
    res.json(appointments);
  } catch (err) {
...
// Doctor's appointment fetching controller-Abdul
export const doctorAppointments = async (req, res) => {
  try {
    ...
    // const doctorGovtId = req.user.id;
...
export const appointmentInvoice = async (req, res) => {
  try {
    ...
    doc.image(path.join(__dirname, "../public/icon.png"), 50, 45, {
      width: 60,
    });
    ...
    const boxY = 180;
    doc.rect(350, boxY, 200, 85).fillAndStroke(bgLight, primaryColor);
    ...
    const addTableRow = (label, value, isEven) => {
      if (isEven) {
        doc.rect(50, currentY - 5, 500, lineHeight).fill(bgLight);
      }
    ...
    const statusColor =
      appointment.paymentStatus === "paid" ? "#10b981" : "#ef4444";
    doc.rect(200, paymentY - 3, 80, 18).fill(statusColor);

    doc
      .fillColor("#ffffff")
      .font("Helvetica-Bold")
      .fontSize(10)
      .text(appointment.paymentStatus.toUpperCase(), 210, paymentY, {
        width: 60,
      });
    ...
    if (appointment.notes) {
      ...
      currentY += 20;
      doc.rect(50, currentY, 500, 50).fill(bgLight);

      doc
        .fillColor(textDark)
        .font("Helvetica")
        .fontSize(8)
        .text(appointment.notes, 60, currentY + 8, {
          width: 480,
          align: "left",
        });
    ...
    doc
      ...
      .fontSize(8)
      .font("Helvetica-Bold")
      .text("VERIFIED", stampX - 25, stampY - 15, {
        width: 50,
        align: "center",
      })
      .text("&", stampX - 10, stampY - 3, { width: 20, align: "center" })
      .text("APPROVED", stampX - 25, stampY + 8, {
        width: 50,
        align: "center",
      });
